        ...
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Plate No Map</h5>                    
                </div>
...
<script>
        ...
        $("#table-records").DataTable({
            ...
            "columns": [
                ...
                {"data": "action","orderable":false,"render": function ( data, type, row, meta ) 
                    {
                       var $val = '<button onclick="edit('+row.id+',\''+row.plate_no+'\',\''+row.desire_plate_no+'\','+row.enable_flag+')" type="button" class="btn btn-link btn-sm btn--icon-text"><i class="zmdi zmdi-edit"></i>Edit</button>';
    ...
    function submit() {
        ...
        var $data = {
            ...
            'desire_plate_no' : $('#input-desire-plate-no').val(),
            'enable_flag' : $('#input-enable_flag').is(":checked")?1:0,
        };
        var $url = 'plate_no_map/create';
        if ($id>0) {
            $url = 'plate_no_map/update';
            $data['id'] = $id;
        }
     ...
     * edit record
     */
    function edit($id,$plate_no,$desire_palte_no,$enable_flag) {
        $('#input-id').val($id);        
        $('#input-plate-no').val($plate_no);
        $('#input-desire-plate-no').val($desire_palte_no);
